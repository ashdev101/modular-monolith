...
const createIntegrationTests = (test, driverFactory) => {
    ...
    test('inserts and retrieves bigint', async (t) => {
        ...
        const result = await pool.oneFirst(__1.sql.unsafe `
      SELECT ${BigInt(9007199254740999n)}::bigint
    `);
        t.is(result, BigInt(9007199254740999n));
        await pool.end();
    });
    ...
    test('properly handles terminated connections', async (t) => {
        ...
        await t.notThrowsAsync(pool.query(__1.sql.unsafe `
    ...
    test('produces syntax error with the original SQL', async (t) => {
        ...
        const error = await t.throwsAsync(pool.any(__1.sql.unsafe `SELECT WHERE`));
    ...
    test('retrieves correct infinity values (with timezone)', async (t) => {
        ...
        await pool.any(__1.sql.unsafe `
      INSERT INTO person (name, updated_at) VALUES ('foo', 'infinity'), ('bar', '-infinity');
    `);
        const result = await pool.any(__1.sql.unsafe `
    ...
    test('inserting records using jsonb_to_recordset', async (t) => {
        ...
        const persons = await pool.any(__1.sql.unsafe `
      INSERT INTO person
      (
        name,
        tags
      ...
      SELECT *
      FROM jsonb_to_recordset(${__1.sql.jsonb([
            {
                name: 'foo',
                tags: ['a', 'b', 'c'],
            },
    ...
    test('re-routes query to a different pool', async (t) => {
        ...
        const readOnlyPool = await (0, __1.createPool)(t.context.dsn, {
            driverFactory,
            interceptors: [
                {
    ...
    test('does not allow to reuse released connection', async (t) => {
        ...
        await t.throwsAsync(firstConnection.oneFirst(__1.sql.unsafe `SELECT 1`));
        await pool.end();
    });
    test('validates results using zod (passes)', async (t) => {
        ...
        const result = await pool.one(__1.sql.type(zod_1.z.object({
            foo: zod_1.z.string(),
        })) `
      SELECT 'bar' foo
    `);
    ...
    test('parses numerics as floats', async (t) => {
        const pool = await (0, __1.createPool)(t.context.dsn, {
            driverFactory,
            typeParsers: [(0, __1.createNumericTypeParser)()],
        });
        const result = await pool.oneFirst(__1.sql.unsafe `
      SELECT 1::numeric foo
    `);
    ...
    test('returns expected query result object (array bytea)', async (t) => {
        ...
        const result = await pool.query(__1.sql.unsafe `
      SELECT ${__1.sql.array([Buffer.from('foo')], 'bytea')} "names"
    `);
        t.deepEqual(result, {
            command: 'SELECT',
            fields: [
    ...
    test('terminated backend produces BackendTerminatedError error', async (t) => {
        ...
        const error = await t.throwsAsync(pool.connect(async (connection) => {
            const connectionPid = await connection.oneFirst(__1.sql.unsafe `
        SELECT pg_backend_pid()
      `);
            setTimeout(() => {
                void pool.query(__1.sql.unsafe `SELECT pg_terminate_backend(${connectionPid})`);
            }, 100);
            await connection.query(__1.sql.unsafe `SELECT pg_sleep(2)`);
        }));
        t.true(error instanceof __1.BackendTerminatedError);
        await pool.end();
    });
    test('cancelled statement produces StatementCancelledError error', async (t) => {
        ...
        `);
            setTimeout(() => {
                void pool.query(__1.sql.unsafe `SELECT pg_cancel_backend(${connectionPid})`);
            }, 100);
            await connection.query(__1.sql.unsafe `SELECT pg_sleep(2)`);
        }));
        t.true(error instanceof __1.StatementCancelledError);
        await pool.end();
    });
    test('statement cancelled because of statement_timeout produces StatementTimeoutError error', async (t) => {
      ...
      `);
            await connection.query(__1.sql.unsafe `SELECT pg_sleep(1)`);
        }));
    ...
    test('throws an error if an attempt is made to make multiple transactions at once using the same connection', async (t) => {
        ...
        t.true(error instanceof __1.UnexpectedStateError);
    ...
    test('writes and reads buffers', async (t) => {
      ...
      (
        'foo',
        ${__1.sql.binary(Buffer.from(payload))}
      )
    `);
        const result = await pool.oneFirst(__1.sql.unsafe `
      SELECT payload
    ...
    test('explicit connection configuration is persisted', async (t) => {
        ...
        await pool.connect(async (connection) => {
            const originalStatementTimeout = await connection.oneFirst(__1.sql.unsafe `SHOW statement_timeout`);
            t.not(originalStatementTimeout, '50ms');
            await connection.query(__1.sql.unsafe `SET statement_timeout=50`);
            const statementTimeout = await connection.oneFirst(__1.sql.unsafe `SHOW statement_timeout`);
    ...
    test('serves waiting requests', async (t) => {
        ...
        const queue = [];
        while (index--) {
            queue.push(pool.query(__1.sql.unsafe `SELECT 1`));
        }
    ...
    test('statements are cancelled after `statementTimeout`', async (t) => {
        ...
        const error = await t.throwsAsync(pool.query(__1.sql.unsafe `SELECT pg_sleep(2000)`));
        t.true(error instanceof __1.StatementTimeoutError);
        await pool.end();
    });
    ...
    test.serial('retries failing transactions (deadlock)', async (t) => {
        ...
        const firstPersonId = await pool.oneFirst(__1.sql.unsafe `
      INSERT INTO person (name)
      VALUES ('foo')
      RETURNING id
    `);
        const secondPersonId = await pool.oneFirst(__1.sql.unsafe `
      INSERT INTO person (name)
      VALUES ('bar')
      RETURNING id
    `);
        ...
        `);
                ...
                await (0, promises_1.setTimeout)(delayDeadlock);
                await deadlock;
                await transaction.query(__1.sql.unsafe `
          UPDATE person
    ...
    test('returns true if returns rows', async (t) => {
        ...
        t.true(await pool.exists(__1.sql.unsafe `
        SELECT LIMIT 1
      `));
        await pool.end();
    });
    test('returns false if returns rows', async (t) => {
        ...
        t.false(await pool.exists(__1.sql.unsafe `
        SELECT LIMIT 0
      `));
        await pool.end();
    });
    ...
    test('throw error with notices', async (t) => {
      ...
      $$;
    `);
        const error = await t.throwsAsync(pool.query(__1.sql.unsafe `SELECT * FROM error_notice(${10});`));
    ...
    test('tuple moved to another partition due to concurrent update error handled', async (t) => {
        ...
        await pool.connect(async (connection) => {
            await connection.query(__1.sql.unsafe `CREATE TABLE foo (a int, b text) PARTITION BY LIST(a)`);
            await connection.query(__1.sql.unsafe `CREATE TABLE foo1 PARTITION OF foo FOR VALUES IN (1)`);
            await connection.query(__1.sql.unsafe `CREATE TABLE foo2 PARTITION OF foo FOR VALUES IN (2)`);
            await connection.query(__1.sql.unsafe `INSERT INTO foo VALUES (1, 'ABC')`);
        });
        await pool.connect(async (connection1) => {
            await pool.connect(async (connection2) => {
                await connection1.query(__1.sql.unsafe `BEGIN`);
                await connection2.query(__1.sql.unsafe `BEGIN`);
                await connection1.query(__1.sql.unsafe `UPDATE foo SET a = 2 WHERE a = 1`);
                connection2
                    .query(__1.sql.unsafe `UPDATE foo SET b = 'XYZ'`)
                    .catch((error) => {
                    t.true(error instanceof __1.TupleMovedToAnotherPartitionError);
                    t.is(error.message, 'Tuple moved to another partition due to concurrent update.');
                });
                // Ensures that query is processed before concurrent commit is called.
                await (0, promises_1.setTimeout)(1000);
                await connection1.query(__1.sql.unsafe `COMMIT`);
                await connection2.query(__1.sql.unsafe `COMMIT`);
            });
        });
        await pool.end();
    });
    ...
    `);
        const error = await t.throwsAsync(pool.query(__1.sql.unsafe `SELECT * FROM invalid_input_error_test where id = '1';`));
        t.true(error instanceof __1.InvalidInputError);
    });
    ...
    `);
        await t.throwsAsync(pool.transaction(async (transaction) => {
            ...
            await transaction.any(__1.sql.unsafe `
        ...
        t.deepEqual(await pool.manyFirst(__1.sql.unsafe `
    ...
    test('command line options are passed to the underlying connection', async (t) => {
        ...
        const pool = await (0, __1.createPool)(t.context.dsn + '?options=' + options, {
            driverFactory,
        });
        await pool.query(__1.sql.unsafe `
    ...
    `);
        ...
        const tableName = await pool.oneFirst(__1.sql.unsafe `
    ...
    `);
      ...
      `));
        t.true(error instanceof __1.CheckIntegrityConstraintViolationError);
    });
    ...
    `);
        ...
        INSERT INTO unique_constraint_test (name) VALUES ('foo');
      `));
        t.true(error instanceof __1.UniqueIntegrityConstraintViolationError);
    });
    ...
    `);
      ...
      `));
        t.true(error instanceof __1.ForeignKeyIntegrityConstraintViolationError);
    });
    ...
    test('terminates transactions that are idle beyond idleInTransactionSessionTimeout', async (t) => {
        ...
        t.true(error instanceof __1.IdleTransactionTimeoutError);
        // Ensure that the pool is still operational.
        t.is(await pool.oneFirst(__1.sql.unsafe `
        SELECT 1;
      `), 1);
    });
    const setIsolationLevel = async (transaction, isolationLevel) => {
    ...
    const testConcurrentTransactions = ({ isolationLevel, expectedResult1, expectedResult2, }) => {
        test('handles concurrent transactions correctly (' + isolationLevel + ')', async (t) => {
            const pool = await (0, __1.createPool)(t.context.dsn, {
                driverFactory,
                maximumPoolSize: 2,
            });
        ...
        `);
            ...
            const transaction1 = pool.transaction(async (transaction) => {
                ...
                await (0, promises_1.setTimeout)(50);
                await transaction.query(__1.sql.unsafe `UPDATE counter SET value = value + 1;`);
                await (0, promises_1.setTimeout)(100);
                return await transaction.oneFirst(__1.sql.unsafe `SELECT value FROM counter;`);
            });
            const transaction2 = pool.transaction(async (transaction) => {
                await setIsolationLevel(transaction, isolationLevel);
                await (0, promises_1.setTimeout)(50);
                await transaction.query(__1.sql.unsafe `UPDATE counter SET value = value + 10;`);
                await (0, promises_1.setTimeout)(50);
                return await transaction.oneFirst(__1.sql.unsafe `SELECT value FROM counter;`);
            });
            await (0, promises_1.setTimeout)(50);
            ...
            const finalCounterValue = await pool.oneFirst(__1.sql.unsafe `SELECT value FROM counter;`);
            t.is(result1, expectedResult1, 'transaction 1 completed with an isolated increment');
            t.is(result2, expectedResult2, 'transaction 2 completed with an isolated increment');
            t.is(finalCounterValue, 11, 'final counter value reflects both transactions');
        });
    };
